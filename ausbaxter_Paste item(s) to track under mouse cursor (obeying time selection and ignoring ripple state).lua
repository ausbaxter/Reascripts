...
function SaveTrackSelection()
    for i = 0, reaper.CountTracks(0)-1 do
        ...
        local sel = reaper.GetMediaTrackInfo_Value(tk, "I_SELECTED")
        if sel then
            table.insert(tbl, tk)
        end
    return tbl
...
function LoadTrackSelection(tbl)
    for i, tk in ipairs(tbl) do
        local sel = reaper.SetMediaTrackInfo_Value(tk, "I_SELECTED", 1)
    end
end
...
function main()
    ...
    hover_track = reaper.BR_GetMouseCursorContext_Track()

    sel_track_table = SaveTrackSelection()
