--[[
 * ReaScript Name: Render Item Columns Through Selected Parent Folder Mono
 ...
 * File URI: https://github.com/ausbaxter/Reascripts/Ausbaxter_Render Item Columns Through Selected Parent Folder Mono.lua
 * Licence: GPL v3
 * REAPER: 5.xx
 * Extensions: None
 * Version: 1.0
...
function GetSelectedItemsinColumns()
  ...
  for i = 0, selItemCount - 1 do
    ...
    if reaper.GetMediaTrackInfo_Value(itemTrack, "B_MAINSEND") == 0 then 
      hasOut = false
      break
    end
  end
...
function SortItemsIntoColumns()
  ...
  table.sort(selectedMediaItems, function(a,b) return a[2] < b[2] end) --sort table in chronological order
  ...
  while loopItemCount <= selItemCount do 
    ...
    while newColumn == false and loopItemCount <= selItemCount do
      local mediaItem = selectedMediaItems[loopItemCount]
...
function Main()
  ...
  if reaper.CountSelectedMediaItems() > 0 and reaper.CountSelectedTracks() == 1 then
    ...
    if renderTrack ~= nil then
      ...
      areChildren, tracksHaveOut = GetSelectedItemsinColumns()
      
      if tracksHaveOut then
    
        if areChildren then
        
          if GetInput() then
            reaper.PreventUIRefresh(1)
            SortItemsIntoColumns()
        ...
        ThrowError("One or more tracks of selected items do not have their Master/Parent Send activated. Items on that track will not be rendered.")
      end
